...
cnn_tokenized_stories_dir = "../myprojectv1/cnn-dailymail-master/cnn_stories_tokenized"
dm_tokenized_stories_dir = "../myprojectv1/cnn-dailymail-master/dm_stories_tokenized"
finished_files_dir = "../myprojectv1/cnn-dailymail-master/finished_files/test"
...
def write_to_json(outdir):
    files = os.listdir(cnn_tokenized_stories_dir)
    i = 0
    for f in files:
        js_example = {}
        js_example['id'] = f.replace('.text', '')
        article_sents, _ = get_art_abs(os.path.join(cnn_tokenized_stories_dir, f))
        js_example['article'] = article_sents
        js_example['abstract'] = ''
        outfile = os.path.join(outdir, '{}.json'.format(i))
        with open(outfile, 'w', encoding='utf-8') as p:
            json.dump(js_example, fp=p, indent=4)
            print("write finished_files success")
...
if __name__ == '__main__':
    ...
    #              makevocab=True)
    write_to_json(finished_files_dir)
